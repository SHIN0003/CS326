...
// TASK #3: Import the new Dictionary module. In addition, you should create a
//         new Dictionary object.
// This is the import from the dictionary module from the previous homework.
// Replace it with an import of the new Dictionary module. In addition, you
// must create a new Dictionary object and assign it to the variable
// `dictionary`.
import Dictionary from "./dictionary.js";
let dict = new Dictionary("dictionary.json");
...
async function possibleWords(availableTiles) {
  // TASK #4: Check if the dictionary has been loaded. If it has not, return an
  //         object with a message and an error. You should use the message
  //         and error from the return value of the `dictionary.get()` method.
  dict.get().then((res) => {
    if (res.status === "error") {
  ...
  const dictionary = await dict.get();
